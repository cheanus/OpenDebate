...
from .utils.llm import llm_score, is_AND_link_reasonable
...
def create_or_opinion(
    ...
    :param is_llm_score: Whether the score is generated by an LLM, which will ignore the provided positive_score.
    :return: The ID of the created opinion or a success message.
    """
    if is_llm_score:
        positive_score = llm_score(content)
...
def create_and_opinion(
    ...
    :param host: The ID of the host (local or external) associated with the opinion.
    :param is_llm_evaluate: Whether to use an LLM to evaluate the reasonableness of the links.
    ...
    # Check if the links are reasonable using LLM
    if is_llm_evaluate:
        son_ids_contents = [son_opinion.content for son_opinion in son_opinion_neo4j_list]
        to_content = parent_opinion_neo4j.content
        if not is_AND_link_reasonable(son_ids_contents, to_content, link_type.value):
            raise ValueError("The AND link is not reasonable according to LLM evaluation.")
